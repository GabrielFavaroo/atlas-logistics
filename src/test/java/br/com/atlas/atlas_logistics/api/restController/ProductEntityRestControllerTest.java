...
class ProductEntityRestControllerTest {
    ...
    void shouldSave() throws Exception {
      var product = new ProductEntity("Carro","AFKAAO",new BigDecimal("100.00"));
...
//      given().pathParam("id",id).header("Authorization","Bearer "+ auditorAccessToken).when().get("/products/{id}").then().assertThat().statusCode(200).and().body("name", equalTo("Carro"));
    ...
    void shouldDelete() throws Exception {
        var product = new ProductEntity("Parafuso","HAUIQOI",new BigDecimal("2.0"));
        String body = objectMapper.writeValueAsString(product);
...
//        given().header("Authorization","Bearer "+ auditorAccessToken).when().get("/products/"+id).then().assertThat().statusCode(404);
    ...
    void shouldUpdate() throws Exception {
        var product = new ProductEntity("Chave de fenda", "HAFASI",new BigDecimal("50.90"));
        String body = objectMapper.writeValueAsString(product);
//
        ...
        var newProduct = new ProductEntity("Chave Philips", "HAFASI", new BigDecimal("55.00"));
...
//                .and().body("value", equalTo(55.00));
    ...
    void shouldPatch() throws Exception {
        ...
        var product = new ProductEntity("Britadeira", "OQOAANA",new BigDecimal("500.00"));
        String body = objectMapper.writeValueAsString(product);
